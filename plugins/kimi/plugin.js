(function () {
  ...
  function loadCredentials(ctx) {
    for (let i = 0; i < CRED_PATHS.length; i += 1) {
      const path = CRED_PATHS[i]
      if (!ctx.host.fs.exists(path)) {
        ctx.host.log.debug("credentials file not found: " + path)
        continue
      }
      ...
      try {
        ...
        if (!parsed || typeof parsed !== "object") {
          ctx.host.log.warn("credentials file is not valid json: " + path)
          continue
        }
        if (!parsed.access_token && !parsed.refresh_token) {
          ctx.host.log.warn("credentials missing access_token and refresh_token: " + path)
          continue
        }
        parsed.__openusage_credential_path = path
        return parsed
      } catch (e) {
    ...
    ctx.host.log.warn("credentials file not found in any known Kimi Code location")
    return null
  }

  function saveCredentials(ctx, creds) {
    const path = creds.__openusage_credential_path || CRED_PATHS[0]
    const out = {}
    for (const key in creds) {
      if (Object.prototype.hasOwnProperty.call(creds, key) && key.indexOf("__openusage_") !== 0) {
        out[key] = creds[key]
      }
    }
    try {
      ctx.host.fs.writeText(path, JSON.stringify(out))
    } catch (e) {
